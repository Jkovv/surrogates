...
def evaluate_windows_comprehensive(model, test_data):
    coords, data_full, time_steps, _ = test_data

    windows = {"Window_82_100": (82, 101), "Window_72_89": (72, 90)}
    report, n_points = {}, coords.shape[0]

    for name, (start, end) in windows.items():
        actual_start, actual_end = int(start), min(int(end), data_full.shape[0])
...
if __name__ == "__main__":
    ...
    for s in seeds:
        mse_val, model = train_and_eval_gpr(study.best_params, train, val, seed=s)
        
        y_tr_pred = model.predict(train[0])
        mse_train = mean_squared_error(train[1], y_tr_pred)
        ...
        all_seed_results.append({
            ...
            "windows": evaluate_windows_comprehensive(model, test)
        })
    ...
    print(f"GPR finished succesfully.")
