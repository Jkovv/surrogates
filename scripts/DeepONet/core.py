...
class MultiScaleFourierProjection(layers.Layer):
    def __init__(self, num_features=128, scales=[0.1, 1.0, 10.0, 100.0], seed=42, **kwargs):
    ...
    def build(self, input_shape):
        ...
        for i, s in enumerate(self.scales):
            B = self.add_weight(
                name=f"B_{i}", 
                shape=(input_shape[-1], n_per_scale),
                initializer=tf.random_normal_initializer(stddev=s, seed=self.seed), 
                trainable=False
            )
...
class DeepONetUncertainty(Model):
    ...
    def call(self, inputs):
        ...
        res = tf.reduce_sum(tf.multiply(b, t), axis=-1)
        return tf.nn.softplus(res)
    ...
    def train_step(self, data):
        ...
        with tf.GradientTape() as tape:
            ...
            log_vars_clipped = tf.clip_by_value(self.log_vars, -2.0, 5.0)
            loss = tf.reduce_sum(tf.exp(-log_vars_clipped) * mse + log_vars_clipped)
...
def build_deeponet(params, grid_size, seed=42, num_cytokines=6):
    act = params.get("activation", "swish")
    lat = params.get("latent_dim", 128)
    tw = params.get("trunk_width", 128)
    nf = params.get("n_filters", 32)
    ...
    bo = layers.Dense(lat*num_cytokines, activation=act)(x)
    bo = layers.Reshape((num_cytokines, lat))(bo)
    
    ti = layers.Input(shape=(3,))
    y = MultiScaleFourierProjection(num_features=128, scales=[0.1, 1.0, 10.0, 100.0], seed=seed)(ti)
    y = layers.Dense(tw, activation=act)(y)
    y = layers.Dense(tw, activation=act)(y)
    to = layers.Dense(lat*num_cytokines, activation=act)(y)
    to = layers.Reshape((num_cytokines, lat))(to)
