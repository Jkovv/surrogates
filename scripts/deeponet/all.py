...
def run_experiment(grid, cytokine, seed):
    ...
    if (out_dir / f"{suffix}.json").exists():
        print(f"Experiment {suffix} already exists. Skipping training.")
        return
    ...
    batch_size = 1024 if grid <= 50 else 256

    def make_dataset(start, end):
        def gen():
            for i in range(start, end):
                target_flat = Y_all[i, ..., idx].flatten()[:, np.newaxis]
                for j in range(grid * grid):
                    yield (X_b[i], X_t[i, j]), target_flat[j]
        
        return tf.data.Dataset.from_generator(
            gen,
            output_signature=((tf.TensorSpec(shape=(grid, grid, X_b.shape[-1]), dtype=tf.float32),
                                tf.TensorSpec(shape=(3,), dtype=tf.float32)),
                               tf.TensorSpec(shape=(1,), dtype=tf.float32))
        ).batch(batch_size).prefetch(tf.data.AUTOTUNE)

    train_ds = make_dataset(0, train_idx)
    val_ds = make_dataset(train_idx, val_idx)
    ...
    def objective(trial):
        ...
        model.fit(train_ds.take(500), epochs=1, verbose=0)
        return model.evaluate(val_ds.take(100), verbose=0)
    ...
    for i in range(n_samples):
        ...
        p = model.predict([b_tile, X_t[i]], verbose=0)
        preds.append(p.reshape(grid, grid))
    preds = np.array(preds)

    res = {
        "params": {"filters": best['n_filters'], "p_dim": best['p_dim'], "L": best['L'], "lr": best['lr']},
        "seed": seed, "grid": grid, "cytokine": cytokine,
        "results": {
    ...
    print(f"Experiment {suffix} complete.")
