...
class DeepONet(tf.keras.Model):
    def __init__(self, grid_size, p=256, scale=15.0):
        super().__init__()
        self.grid_size = grid_size
        self.p = p
        
        self.B = tf.Variable(
            tf.random.normal([3, 128], stddev=scale), 
            trainable=False, 
            name="fourier_B"
        )
        ...
        self.branch = tf.keras.Sequential([
            tf.keras.layers.TimeDistributed(tf.keras.layers.Conv2D(32, 3, strides=2, padding='same', activation='relu')),
            tf.keras.layers.TimeDistributed(tf.keras.layers.Conv2D(64, 3, strides=2, padding='same', activation='relu')),
    ...
    def fourier_map(self, x):
        # Maps raw coordinates to high-dimensional sine/cosine space
    ...
    def call(self, inputs):
        ...
        coords_fourier = self.fourier_map(inputs[1])
        x_t = self.trunk(coords_fourier)
        res = tf.einsum("bp,bnp->bn", x_b, x_t) + self.b
...
def run_pipeline(grid, seed, cytokine):
    ...
    out_dir = Path(f"./models/deeponet_f")
    ...
    print(f"Training DeepONet (Fourier) for {cytokine}...")
    model.fit([X_b[:80], X_t[:80]], Y[:80], epochs=100, batch_size=4, verbose=1)
    ...
    print(f"DONE: {suffix} saved in deeponet_f/")
