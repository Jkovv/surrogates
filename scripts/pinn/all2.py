...
def t_to_norm(t_idx, n_total=201):
...
def run_pipeline(grid, seed, cytokine):
    ...
    out_dir   = Path("./models_200h/pinn"); out_dir.mkdir(parents=True, exist_ok=True)
    ...
    masks_mean = M_pinn[:140].mean(axis=0)  # (G*G, 5)
    ...
    t_norm_all = np.linspace(-1.0, 1.0, 201, dtype=np.float64)
    ...
    val_indices   = list(range(140, 160))
    ...
    print(f"  Train time: {train_elapsed:.1f}s | Pred time: {pred_elapsed:.1f}s")
    ...
    results = {
        ...
        "results": {
            "Near_Horizon_t162_t181": calculate_metrics(
                Y_phys[160:180], Yp_phys[160:180], M_spat[160:180], clip_max),
            "Far_Horizon_t182_t200": calculate_metrics(
                Y_phys[180:199], Yp_phys[180:199], M_spat[180:199], clip_max),
        },
    }
