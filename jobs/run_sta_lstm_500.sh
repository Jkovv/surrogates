...
#SBATCH --array=0-5
#SBATCH --output=jobs/logs/sta_lstm_500_%a.out
#SBATCH --error=jobs/logs/sta_lstm_500_%a.err
...
RESULT_FILE="models/sta_lstm/res_${CYT}_500_${SEED}.json"
...
/gpfs/home3/rbumbuc1/codes/students/surrogates/.venv/bin/python scripts/sta_lstm/all_500.py --cytokine $CYT --seed $SEED --data-dir "$DATA_DIR"

EXIT_CODE=$?
echo "Finished at $(date) | exit code: $EXIT_CODE"
if [ $EXIT_CODE -eq 0 ]; then
    echo "Result saved: models/sta_lstm/res_${CYT}_500_${SEED}.json"
    cat models/sta_lstm/res_${CYT}_500_${SEED}.json
else
