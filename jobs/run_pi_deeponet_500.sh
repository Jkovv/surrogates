#!/bin/bash
#SBATCH --job-name=pi_deeponet_500
...
#SBATCH --array=0-5
#SBATCH --output=jobs/logs/pi_deeponet_500_%a.out
#SBATCH --error=jobs/logs/pi_deeponet_500_%a.err
...
echo "Model:    pi_deeponet 500x500"
...
mkdir -p models/pi_deeponet jobs/logs
...
RESULT_FILE="models/pi_deeponet/res_${CYT}_500_${SEED}.json"
...
/gpfs/home3/rbumbuc1/codes/students/surrogates/.venv/bin/python scripts/pi_deeponet/all_500.py --cytokine $CYT --seed $SEED --data-dir "$DATA_DIR"

EXIT_CODE=$?
echo "Finished at $(date) | exit code: $EXIT_CODE"
if [ $EXIT_CODE -eq 0 ]; then
    echo "Result saved: models/pi_deeponet/res_${CYT}_500_${SEED}.json"
    cat models/pi_deeponet/res_${CYT}_500_${SEED}.json
else
